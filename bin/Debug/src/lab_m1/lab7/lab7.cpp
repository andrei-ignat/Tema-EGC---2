...
void Lab7::Init()
{
    GetSceneCamera()->SetPosition(glm::vec3(0, 5, 3)); // Pozi?ia ini?ialã a camerei
...
void Lab7::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        // Control light position using W, A, S, D, E, Q
        ...
        // Control camera movement using 1 and 2
        if (window->KeyHold(GLFW_KEY_1)) {
            GetSceneCamera()->MoveForward(deltaTime * speed);
            GetSceneCamera()->Update(); // Actualizeazã camera
        }
