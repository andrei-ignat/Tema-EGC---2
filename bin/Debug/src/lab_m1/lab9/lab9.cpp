...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    // Pozi?ia quadului
    glm::vec3 quadPosition = glm::vec3(0.0f, 0.5f, 0.0f); // Aceasta ar trebui sã fie pozi?ia unde ai plasat quadul

    // Calcularea vectorului direc?ie în planul XZ (ignorãm componenta Y)
    glm::vec3 direction = glm::normalize(glm::vec3(cameraPosition.x - quadPosition.x, 0, cameraPosition.z - quadPosition.z));

    // Calcularea unghiului de rota?ie folosind arctan2
    float angle = atan2(direction.z, direction.x);

    // Crearea matricei de model pentru quad
    glm::mat4 modelMatrix = glm::mat4(1);
    modelMatrix = glm::translate(modelMatrix, quadPosition);
    modelMatrix = glm::rotate(modelMatrix, glm::pi<float>() * 0.5f-angle, glm::vec3(0, 1, 0)); // Rote?te doar pe OY
    modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));

    // Renderizare quad
    RenderSimpleMesh(meshes["square"], shaders["LabShader"], modelMatrix, mapTextures["grass"]);

    // Alte obiecte din scenã
    {
        glm::mat4 sphereModelMatrix = glm::mat4(1);
        sphereModelMatrix = glm::translate(sphereModelMatrix, glm::vec3(1, 1, -3));
        sphereModelMatrix = glm::scale(sphereModelMatrix, glm::vec3(2));
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], sphereModelMatrix, mapTextures["earth"], mapTextures["grass"]);
    }

    {
        glm::mat4 boxModelMatrix = glm::mat4(1);
        boxModelMatrix = glm::translate(boxModelMatrix, glm::vec3(2, 0.5f, 0));
        boxModelMatrix = glm::rotate(boxModelMatrix, RADIANS(60.0f), glm::vec3(1, 0, 0));
        boxModelMatrix = glm::scale(boxModelMatrix, glm::vec3(0.75f));
...
void Lab9::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    glUniform1f(glGetUniformLocation(shader->program, "elapsedTime"), static_cast<float>(Engine::GetElapsedTime()));



    int isEarth = (mesh->GetMeshID() == "sphere") ? 1 : 0;
    glUniform1i(glGetUniformLocation(shader->program, "isEarth"), isEarth);
